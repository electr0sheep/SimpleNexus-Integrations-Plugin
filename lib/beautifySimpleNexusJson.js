...
module.exports = {
  beautifySimpleNexusJson: function (jsonFields) {
    ...
    if (editor = atom.workspace.getActiveTextEditor()) {
      ...
      // Multi-Phase
      else if (parsedJson.structure[0] !== undefined) {
      ...
      for (var i = 0; i < cleanedJson.fields.length; i++) {
        // check for no title
        if (cleanedJson.fields[i].title === undefined) {
          atom.notifications.addWarning("Field " + cleanedJson.fields[i].key + " has not title!")
        }

        // check for allowAllStates
        if (cleanedJson.fields[i].type == "state") {
          if (cleanedJson.fields[i].key == "property_state") {
            if (cleanedJson.fields[i].allowAllStates && cleanedJson.fields[i].allowAllStates == true) {
              atom.notifications.addWarning("Field " + cleanedJson.fields[i].key + " is subject property state, but allows all states!")
            }
          } else {
            if (cleanedJson.fields[i].allowAllStates === undefined || (cleanedJson.fields[i].allowAllStates && cleanedJson.fields[i].allowAllStates == false)) {
              atom.notifications.addWarning("Field " + cleanedJson.fields[i].key + " is of state type but does not allow all states!")
            }
          }
        }

        // check for single choice fields with no choices
        if (cleanedJson.fields[i].type == "single_choice") {
          if (cleanedJson.fields[i].choices === undefined) {
            atom.notifications.addError("Field " + cleanedJson.fields[i].key + " is of single choice type but has no choices!", {dismissable: true})
            has_errors = true
          }
        }

        // check for single choice fields with no blank option at the start
        if (cleanedJson.fields[i].type == "single_choice") {
          if (cleanedJson.fields[i].choices && cleanedJson.fields[i].choices[0] != "") {
            atom.notifications.addWarning("Field " + cleanedJson.fields[i].key + " is of single choice type but has no blank option!")
          }
        }

        // check for multi choice fields with no choices
        if (cleanedJson.fields[i].type == "multi_choice") {
          if (cleanedJson.fields[i].choices === undefined) {
            atom.notifications.addError("Field " + cleanedJson.fields[i].key + " is of multi choice type but has no choices!", {dismissable: true})
            has_errors = true
          }
        }
      }
      ...
      if (editor.getText() !== JSON.stringify(cleanedJson, null, 2) + '\n') {
