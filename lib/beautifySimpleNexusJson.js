...
module.exports = {
  beautifySimpleNexusJson: function (jsonFields) {
    let editor
    let nmm = atom.config.get("sn-integrations-plugin.showConfirmationDialog")
          ...
          if (fieldExists === false) {
            ...
            if (nmm) {
              ...
              if (nmmHasField === false) {
                atom.notifications.addError("NMM couldn't find \'" + parsedJson.structure[i].fields[i2] + "\' please create a new issue: https://github.com/electr0sheep/sn-integrations-plugin/issues/new")
              }
            } else {
