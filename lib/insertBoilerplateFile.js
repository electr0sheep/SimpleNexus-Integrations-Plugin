  ...
  insertSimpleNexusTextFromFile: function (path, successNotificationText) {
    ...
    if (editor = atom.workspace.getActiveTextEditor()) {
      if (editor.getPath() != null) {
        ...
        atom.notifications.addError("Editor didn't have a path!", {dismissable: true})
      }
    }
  }
};
...
function askFileExtensionDialog(path, editor, successNotificationText) {
  let myFirstPromise = new Promise((resolve, reject) => {
    if (atom.config.get("SimpleNexus-Integrations-Plugin.showConfirmationDialog")) {
      atom.confirm({
        type: 'warning',
        ...
        if (checkboxChecked) {
          atom.config.set("SimpleNexus-Integrations-Plugin.showConfirmationDialog", false)
        }
        resolve()
      })
    }
