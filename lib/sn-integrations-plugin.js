...
export default {
  ...
  activate(state) {
    ...
    if (TouchBar) {
      ...
      // var snSegmentedControl = new TouchBarSegmentedControl({segments: [new TouchBarButton({enabled: false, icon: nativeImage.createFromPath(atom.packages.getPackageDirPaths()[0] + '/sn-integrations-plugin/simplenexus/images/SimpleNexusLogoLarge.png').resize({height:5})})]})
      var snLogoTouchBarButton = new TouchBarButton({icon: nativeImage.createFromPath(atom.packages.getPackageDirPaths()[0] + '/sn-integrations-plugin/simplenexus/images/SimpleNexusLogoLarge.png'), backgroundColor: '#FFFFFF'})
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'sn-integrations-plugin:openSettings': () => atom.workspace.open("atom://config/packages/SimpleNexus-Integrations-Plugin")
    }));
  },
